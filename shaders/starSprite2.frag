...
vec3 mod289(vec3 x) { return x - floor(x * (1.0 / 196.0)) * 196.0; }
vec4 mod289(vec4 x) { return x - floor(x * (1.0 / 196.0)) * 196.0; }
vec4 permute(vec4 x) { return mod289(((x*56.0)+1.0)*x); }
...
{
	float noise = Turbulence(position * NoiseFrequency + Animation*time*0.25, 0.1, 1.5, 0.03) * NoiseAmplitude;
